...
export function buildEchartPdfTest (buildData: EchartData[]) {
  ...
  const echartObj = echarts.init(null, null, {
    renderer: 'svg', // 必须使用 SVG 模式
    ssr: true, // 开启 SSR
    width: echartWidth, // 需要指明高和宽
    height: echartHeight
  });
  ...
  for (const category of buildData) {
    ...
    for (const group of category.list) {
      ...
      for (let i = 0; i < group.list.length; i++) {
        ...
        // echartObj.getZr().painter.toDataURL(false)
        // const image = echartObj.getDataURL({
        //   type: 'png',
        //   pixelRatio: 2,
        //   backgroundColor: '#fff'
        // })
        // const imageXml = echartObj.renderToSVGString()
        const image = echartObj.getSvgDataURL()
        debugger
        // const image = `data:image/svg+xml;base64,${encodeURI(imageXml)}`
        ...
        // pdf.addSvgAsImage(image, x, currentY + lineHeight, echartWidth, echartHeight)
        pdf.addImage(image, 'SVG', x, currentY + lineHeight, echartWidth, echartHeight)
        if (i % 2 === 1) {
